...
event zeek_init()
{
    ...
    local r2 = SumStats::Reducer($stream="count of 404 resp", $apply=set(SumStats::SUM));
    local r3 = SumStats::Reducer($stream="count of uni 404 resp", $apply=set(SumStats::UNIQUE));
    SumStats::create([$name = "detect attacker through 404 resp", $epoch = 10min, $reducers = set(r1,r2,r3), $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) =
                        {
